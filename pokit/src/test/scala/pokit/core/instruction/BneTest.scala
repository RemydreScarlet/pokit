...
class BneTest extends AnyFlatSpec with ChiselScalatestTester {
    "BNE instruction" should "branch if not equal" in {
        test(new Pipeline) { dut =>
            ...
            // BNE x2, x3, 0 -> 0x00311063 (taken since 10 != 20)
            val instr = "h00311063".U
